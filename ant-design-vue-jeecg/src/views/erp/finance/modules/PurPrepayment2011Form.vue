    ...
    <div>
      <a-form-model ref="form" :model="model" :rules="validatorRules">
        ...
        <a-tabs v-model="activeKey" @change="handleChangeTabs">
          <a-tab-pane tab="采购预付申请单" :key="refKeys[1]" :forceRender="true">
            <j-vxe-table
              keep-source
              :ref="refKeys[1]"
            ...
            >
              <template v-if="!disabled" v-slot:toolbarPrefix>
                <a-tooltip :title="tipTitleOfAdd" placement="bottom">
                  <a-button @click="$refs.reqPopup.openModal()" icon="plus" type="primary" :disabled="addDisabled">预付申请单</a-button>
                </a-tooltip>
                ...
                <j-popup v-show="false" ref="reqPopup" code="fin_payment_req" :param="reqPopupParam"
  ...
  export default {
    ...
    data() {
      return {
        ...
        entryTable: {
          ...
          selectedRowCount: 0,
          dataSource: [],
          url: {list: '/finance/finPayment/queryEntryByMainId'},
          columns: [
            {
              title: '#',
              key: 'entryNo',
        ...
        srcTable: {
          loading: false,
          dataSource: [],
          url: {list: '/finance/finPaymentReq/listByIds'},
          columns: [
            {
              title: '单据编号',
              key: 'billNo',
        ...
        url: {
          ...
          queryById: "/finance/finPayment/queryById",
          check: "/finance/finPayment/check",
          ebpm: "/finance/finPayment/bpm/end",
          execute: "/finance/finPayment/execute",
          void: "/finance/finPayment/void",
        },
      }
    },
    ...
    computed: {
      tipTitleOfAdd() {
        ...
        if (this.entryTable.rowCount > 0) return "只能添加一个采购预付申请单！";
        return "供应商是弹窗查询参数";
      },

      addDisabled() {
        return !this.model.supplierId || this.model.supplierId.length === 0 || this.entryTable.rowCount > 0;
      },

      reqPopupParam() {
        const v = {payment_type: '2011', is_closed: 0};
    ...
    methods: {
      ...
      addAfter() {
        this.$refs.billHeader.fillBillNo('fin_cgyf_bill_no');
      },
      ...
      classifyIntoFormData(allValues) {
        ...
        return {
          ...main, // 展开
          finPaymentEntryList: allValues.tablesValue[0].tableData,
        }
      },
          ...
          for (let srcBill of deltas.srcBills) {
            if (!entries.find(e => e.srcBillId === srcBill.id))
              jxTable.addRows({srcBillType: srcBill.billType, srcBillId: srcBill.id, srcNo: srcBill.billNo,
                amt: srcBill.amt - srcBill.paidAmt}).then(
      ...
      amtValidator({cellValue, row, column}, callback, target) {
        ...
        if (v > row1.amt - row1.paidAmt) {
          callback(false, '${title}不能大于未付金额');
        } else {
