    ...
    <div>
      <a-form-model ref="form" :model="model" :rules="validatorRules">
        ...
        <a-tabs v-model="activeKey" @change="handleChangeTabs">
          <a-tab-pane tab="销售出库单" :key="refKeys[1]" :forceRender="true">
            <j-vxe-table
              keep-source
              :ref="refKeys[1]"
            ...
            >
              <template v-if="!disabled" v-slot:toolbarPrefix>
                <a-tooltip :title="!model.customerId || model.customerId.length===0 ? '请先选择客户！':'客户是弹窗查询参数'"
                           placement="bottom">
                  <a-button @click="$refs.outPopup.openModal()" icon="plus" type="primary"
                            :disabled="!model.customerId || model.customerId.length===0">销售出库单</a-button>
                </a-tooltip>
                ...
                <j-popup v-show="false" ref="outPopup" code="stk_out_bill" :param="outPopupParam"
                         org-fields="id" dest-fields="id" :multi="true" @input="onOutPopupInput"/>
              </template>
            </j-vxe-table>
          </a-tab-pane>
...
<script>
  ...
  export default {
    ...
    computed: {
      outPopupParam() {
        const v = {stock_io_type: '201', is_closed: 0};
    ...
    methods: {
      ...
      addAfter() {
        this.$refs.billHeader.fillBillNo('fin_xssk_bill_no');
      },
