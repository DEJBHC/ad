            ...
            >
              <template v-if="!disabled" v-slot:toolbarPrefix>
                <a-tooltip :title="!model.supplierId || model.supplierId.length===0 ? '请先选择供应商！':'供应商是弹窗查询参数'"
                           placement="bottom">
                  <a-button @click="$refs.inPopup.openModal()" icon="plus" type="primary"
                            :disabled="!model.supplierId || model.supplierId.length===0">采购入库单</a-button>
                </a-tooltip>
                ...
                <j-popup v-show="false" ref="inPopup" code="stk_in_bill" :param="inPopupParam"
                         org-fields="id" dest-fields="id" :multi="true" @input="onInPopupInput"/>
              </template>
            </j-vxe-table>
          </a-tab-pane>
...
<script>
  ...
  export default {
    ...
    computed: {
      inPopupParam() {
        const v = {stock_io_type: '101', is_closed: 0};
    ...
    methods: {
      ...
      amtValidator({cellValue, row, column}, callback, target) {
        ...
        if (v > row1.settleAmt - row1.settledAmt) {
          callback(false, '${title}不能大于未结算金额');
        } else {
