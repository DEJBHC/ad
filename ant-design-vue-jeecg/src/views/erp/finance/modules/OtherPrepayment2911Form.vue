    ...
    <div>
      <a-form-model ref="form" :model="model" :rules="validatorRules">
        ...
        <a-tabs v-model="activeKey" @change="handleChangeTabs">
          <a-tab-pane tab="预付申请" :key="refKeys[1]" :forceRender="true">
            <j-vxe-table
              keep-source
              :ref="refKeys[1]"
              ...
              <template v-if="!disabled" v-slot:toolbarPrefix>
                <a-tooltip :title="!model.supplierId || model.supplierId.length===0 ? '请先选择供应商！':'供应商是弹窗查询参数'"
                  ...
                  <a-button @click="$refs.reqPopup.openModal()" icon="plus" type="primary"
                            :disabled="!model.supplierId || model.supplierId.length===0">预付申请</a-button>
                </a-tooltip>
...
<script>
  ...
  export default {
    ...
    computed: {
      reqPopupParam() {
        const v = {payment_type: '2911', is_closed: 0};
    ...
    methods: {
      ...
      addAfter() {
        this.$refs.billHeader.fillBillNo('fin_qtyf_bill_no');
      },
