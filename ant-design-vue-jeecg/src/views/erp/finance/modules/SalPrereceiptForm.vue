    ...
    <div>
      <a-form-model ref="form" :model="model" :rules="validatorRules">
        ...
        <a-tabs v-model="activeKey" @change="handleChangeTabs">
          <a-tab-pane tab="销售订单" :key="refKeys[1]" :forceRender="true">
            <j-vxe-table
              keep-source
              :ref="refKeys[1]"
            ...
            >
              <template v-if="!disabled" v-slot:toolbarPrefix>
                <a-tooltip :title="tipTitleOfAdd" placement="bottom">
                  <a-button @click="$refs.orderPopup.openModal()" icon="plus" type="primary" :disabled="addDisabled">销售订单</a-button>
                </a-tooltip>
                ...
                <j-popup v-show="false" ref="orderPopup" code="sal_order" :param="orderPopupParam"
  ...
  export default {
    ...
    data() {
      return {
        ...
        srcTable: {
          loading: false,
          dataSource: [],
          url: {list: '/sale/salOrder/listByIds'},
          columns: [
            {
              title: '单据编号',
              key: 'billNo',
            ...
            },
            {
              ...
              key: 'customerId',
              type: JVXETypes.select,
              dictCode:"bas_customer,aux_name,id",
              width:"160px",
              sortable: true,
            },
            {
    ...
    computed: {
      tipTitleOfAdd() {
        ...
        if (this.entryTable.rowCount > 0) return "只能添加一个销售订单！";
        return "客户是弹窗查询参数";
      },

      addDisabled() {
        return !this.model.customerId || this.model.customerId.length === 0 || this.entryTable.rowCount > 0;
      },
    ...
    methods: {
      ...
      addAfter() {
        this.$refs.billHeader.fillBillNo('fin_xsys_bill_no');
      },
          ...
          for (let srcBill of deltas.srcBills) {
            if (!entries.find(e => e.srcBillId === srcBill.id))
              jxTable.addRows({srcBillType: srcBill.billType, srcBillId: srcBill.id, srcNo: srcBill.billNo,
                amt: srcBill.amt - srcBill.prereceiptBal - srcBill.settledAmt}).then(
      ...
      amtValidator({cellValue, row, column}, callback, target) {
        ...
        if (v > row1.amt - row1.prereceiptBal - row1.settledAmt) {
          callback(false, '${title}不能大于（销售金额 - 预收余额 - 已结算金额）');
        } else {
