      ...
      if (statistics.has) {
        if (statistics.sum.length > 0) {
          footers.push(this.getFooterStatisticsMap({
            ...
            // 20220410 cfm modi: 统计应用列的格式化
            // method: (column) => XEUtils.sum(data, column.property)
            method: (column) => {
              let cellValue = XEUtils.sum(data, column.property);
              return !column.formatter ?  cellValue : column.formatter({cellValue})
            }
          }))
        }
        if (statistics.average.length > 0) {
          footers.push(this.getFooterStatisticsMap({
            ...
            method: (column) => {
              let cellValue = XEUtils.mean(data, column.property);
              return !column.formatter ?  cellValue : column.formatter({cellValue})
            }
          }))
        }
          ...
          on: {
            ...
            remove: () => {
              ...
              if (deleteRows.length > 0) {
                ...
                //20220512 cfm warn: 参数deleteRows未包含新行！
                this.trigger('remove', removeEvent)
              } else {
                this.removeSelection()
                //20220512 cfm add&warn：未带参数
                this.trigger('remove')
              }
            },
            // 清除选择事件
