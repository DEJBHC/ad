...
public class SysThirdAccountServiceImpl extends ServiceImpl<SysThirdAccountMapper, SysThirdAccount> implements ISysThirdAccountService {
    ...
    public void updateThirdUserId(SysUser sysUser,String thirdUserUuid) {
        ...
        //20220114 cfm todo: 不同第三方的thirdUserUuid有可能相同的隐患！暂不影响
        ...
        //20220115 cfm: 本段代码为“如果sysUser已绑定同一第三方的另一账号，则删除”
        //20220115 cfm todo：如果thirdUserUuid已绑定sysUser，则后续的update将出问题！
    ...
    public SysUser createUser(String phone, String thirdUserUuid) {
       //先查询第三方，获取登录方式
        ...
        SysThirdAccount account = sysThirdAccountMapper.selectOne(query);
        //通过用户名查询数据库是否已存在
        //20220115 cfm todo：应使用selectById(...) 或 getUserByPhone(...)，暂不影响是因为调用本方法之处都先用手机号判断用户不存在！
    ...
    public SysThirdAccount saveThirdUser(ThirdLoginModel tlm) {
        ...
        //2022016 cfm modi
        //user.setThirdUserId(tlm.getUserId());
        String userId = tlm.getUserId();
        if (oConvertUtils.isEmpty(userId)) {
            userId = this.getUserIdByUuid(tlm.getUuid(), tlm.getSource());
        }
    ...
    //2022016 cfm add
    private String getUserIdByUuid(String uuid, String source){
        switch (source.toUpperCase()) {
            case ThirdAppConfig.DINGTALK:
                return dingtalkService.getUserIdByUnoinId(uuid);
            case ThirdAppConfig.WECHAT_ENTERPRISE:
                return uuid;
            case ThirdAppConfig.WECHAT_MP:
                return null;
            default:
                return null;
        }
