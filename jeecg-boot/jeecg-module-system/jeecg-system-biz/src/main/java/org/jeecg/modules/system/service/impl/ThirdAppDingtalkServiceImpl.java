...
public class ThirdAppDingtalkServiceImpl implements IThirdAppService {
    ...
    //20220117 cfm add
    @Autowired
    private ISysThirdDepartService sysThirdDepartService;

    //20220116 cfm add
    public String getUserIdByUnoinId(String unionId) {
        ...
        return getUserIdRes.getResult();
    }

    //20220117 cfm add
    private void refreshSysThirdDepart(List<Department> dtDepartmentList) {
        sysThirdDepartService.removeByThirdType(this.THIRD_TYPE);
        for (Department dt : dtDepartmentList){
            SysThirdDepart std = new SysThirdDepart();
            std.setSysDepartId(dt.getSource_identifier());
            std.setThirdDepartId(dt.getDept_id().toString());
            std.setThirdType(this.THIRD_TYPE);
            sysThirdDepartService.save(std);
        }
    }
    ...
    public SyncInfoVo syncLocalDepartmentToThirdApp(String ids) {
        ...
        //20220117 cfm add
        refreshSysThirdDepart(JdtDepartmentAPI.listAll(accessToken));
    ...
    public SysUser oauth2Login(String authCode) {
        ...
        if (appUserId != null) {
            ...
            if (thirdAccount != null) {
                ...
                //20220115 cfm modi: 增加参数 appUser.getUnionid()
                ThirdLoginModel tlm = new ThirdLoginModel(THIRD_TYPE, appUser.getUnionid(), appUser.getUserid(), appUser.getName(), appUser.getAvatar());
    ...
    private SysUser getSysUserByThird(SysThirdAccount thirdAccount, User appUser, String appUserId, String accessToken) {
        ...
        if (oConvertUtils.isNotEmpty(sysUserId)) {
            ...
            if (sysUser != null) {
                ...
                //20220115 cfm: 与原SysThirdAccountServiceImpl.saveThirdUser()不一致！
                thirdAccount.setThirdUserId(appUser.getUserid());

                //20220115 cfm modi: appUser.getUserid() to appUser.getUnionid()
